...
class AddaSearchEngine:
    ...
    def _synthesize(self, query: str, context_docs: List[str], extracted_entities: Dict = None) -> str:
        """Steg 5: Syntes - med injicerad session state"""
    ...
    def run(self, query: str, history: List[Dict] = []) -> Dict[str, Any]:
        """Main Pipeline"""
        logger.info(f"New Query: {query}")
        
        # 0. EXTRACT SESSION STATE (Shadow State)
        # This runs BEFORE planning to understand what we know so far
        current_state = self._extract_session_state(query, history)
        ...
        # 5. SYNTHESIZE (med extracted_entities för att undvika att fråga igen)
        answer = self._synthesize(query, context_docs, extracted_entities)

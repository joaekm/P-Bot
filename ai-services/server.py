...
try:
    ...
    logger.info("AddaSearchEngine initialized successfully")
...
def conversation():
    ...
    try:
        ...
        # Run the engine
        # Note: AddaSearchEngine.run() expects (query, history)
        # We might need to adapt history format if frontend sends different structure
        # Frontend sends: [{role: 'user'|'assistant', content: '...'}] which matches backend expectation mostly.
        
        result = engine.run(user_message, history)
        ...
        response = {
            ...
            "ui_directives": ui_directives  # Frontend rendering hints
        }
